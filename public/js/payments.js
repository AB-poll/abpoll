...
function subscribeModalNext(){
    if (document.getElementById('subscribeSelectedPrice').innerHTML == "$12.99"){
        window.location = "/subscription/create?plan=monthly";
    } else {
        window.location = "/subscription/create?plan=yearly";
    }
}
...
var cvcNumber = document.getElementById("card-cvc");
if (cvcNumber){
    var cvcNumber = document.getElementById("card-cvc");
    // first add the event listener for oninput. oninput is triggered whenever an element is changed.
    cvcNumber.addEventListener("input", function(event){
      if (cvcNumber.value.length > 4) {
        // if the inputted value is more than 4 then set it to the max
        cvcNumber.value = cvcNumber.value.slice(0,4);
      }
      // set the value to a base 10 number. that way if a user inputs a decimal point it will be removed.
      cvcNumber.value = Number(cvcNumber.value);
    });
}
